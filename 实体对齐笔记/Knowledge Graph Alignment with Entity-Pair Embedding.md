...
使用**局部敏感哈希Locality-Sensitive Hashing (LSH)**(其基本的思想就是通过哈希函数将输入的高维特征矢量散列至低维特征空间，并满足在原始空间中距离较近的点经过散列之后在低维空间依然距离较近，距离较近的点散列后碰撞的概率要大于距离较远的点碰撞的概率。)将特征向量映射到低维空间。

计算两个特征向量的Jaccard相似性，大于预设阈值则进行配对生成PCG的节点和边。
...
$A_j({e}')$是实体${e}’$的第$j$个属性值，将所有类型的属性值都视为字符串

$M_{m \times n}$是相似性矩阵，其中元素$m_{ij}$代表两个属性的相似度，采用基于N-gram的Jaccard相似度
$$
Jaccard(s,t)=\frac{|NG(s) \cap NG(t)|}{|NG(s) \cup NG(t)|}
$$
其中NG(s)表示字符串s的n元字符串集合

由于实体通常只有知识图谱所有属性中的少量属性，因此这个矩阵是很稀疏的，其中由很多的0。因此使用CNN将相似矩阵转换为一个稠密向量
$$
